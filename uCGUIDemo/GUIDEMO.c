...
static fVoidVoid* _apfTest[] =
{
   ...
   //      GUIDEMO_Touch,
   #endif
   GUIDEMO_DemoProgBar, GUIDEMO_DemoFrameWin, GUIDEMO_Dialog,
   //    GUIDEMO_WM,
   #endif
   0
};
...
int GUIDEMO_WaitKey(void)
{
   ...
   PROGBAR_Handle hProg = PROGBAR_Create(LCD_GetXSize() - 70,
                           LCD_GetYSize() - 40, 80, 5, WM_CF_SHOW);
#endif
...
void GUIDEMO_ShowIntro(const char* s, const char* sExp)
{
   ...
   {
      GUI_SetFont(&GUI_Font8_ASCII);
      GUI_DispStringHCenterAt(_sExplain, LCD_GET_XSIZE() / 2,
         LCD_GET_YSIZE() / 2 - 10);
   }
...
void GUIDEMO_main(void)
{
   ...
   _ahFrameWin[1] = FRAMEWIN_Create("Control", NULL,
                     WM_CF_SHOW | WM_CF_STAYONTOP, LCD_GetXSize() - 80,
                     LCD_GetYSize() - 60, 80, 60);
   _ahInfoWin[1] = WM_CreateWindowAsChild(0, 0, 0, 0,
                     WM_GetFirstChild(_ahFrameWin[1]),
                     WM_CF_SHOW | WM_CF_STAYONTOP, &_cbCmdWin, 0);
   _ahButton[0] = BUTTON_CreateAsChild(4, 20, _ButtonSizeX, _ButtonSizeY,
                     _ahInfoWin[1], 'S',
                     WM_CF_SHOW | WM_CF_STAYONTOP | WM_CF_MEMDEV);
   _ahButton[1] = BUTTON_CreateAsChild(40, 20, _ButtonSizeX, _ButtonSizeY,
                     _ahInfoWin[1], 'N',
                     WM_CF_SHOW | WM_CF_STAYONTOP | WM_CF_MEMDEV);
...
void GUIDEMO_main(void)
{
   ...
   _ahButton[0] = BUTTON_Create(LCD_GetXSize() - _ButtonSizeX * 2 - 5,
                     LCD_GetYSize() - _ButtonSizeY - 3, _ButtonSizeX,
                     _ButtonSizeY, 'H',
                     WM_CF_SHOW | WM_CF_STAYONTOP | WM_CF_MEMDEV);
