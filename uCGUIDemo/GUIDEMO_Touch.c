...
static void _Calibrate(int Coord, int Log0, int Log1, int Phys0, int Phys1,
   int* p0, int* p1)
{
   int l0 = 0;
...
//   int ax[2] =
//   {
//      LCD_XSIZE / 10, LCD_XSIZE - LCD_XSIZE / 10
//   };
//   int ay[2] =
//   {
//      LCD_YSIZE - LCD_YSIZE / 10, LCD_YSIZE / 10
//   };
...
static char _acText[] =
{
   '1', '2', '3', '4', '5', '6', '7', '8', '9', '0', '=', 0, 'Q', 'W', 'E',
   'R', 'T', 'Y', 'U', 'I', 'O', 'P', 0, 'A', 'S', 'D', 'F', 'G', 'H', 'J',
   'K', 'L', ':', 0, 'Z', 'X', 'C', 'V', 'B', 'N', 'M', '<', '>', '?', ' ',
   ' ', ' ', ' ', ' ', 0, 0
};
...
int _ExecKeyboard(void)
{
   ...
   rText.y0 += 20;
   rText.y1 += 20;
   GUI_DispStringInRect("Touch screen demo", &rText,
...
int _ExecKeyboard(void)
{
   ...
   for (i = 0; _acText[i]; i++)
   {
      ...
      char* s = ac;
      ac[0] = c;
      ahButton[i] = BUTTON_Create(XPos, YPos, XStep - 5, YStep - 5, Id,
                     BUTTON_CF_SHOW);
      BUTTON_SetText(ahButton[i], s);
      BUTTON_EnableMemdev(ahButton[i]);
   }
...
#define ID_CALIBRATE 3
   ...
   do
   {
      ...
      GUI_SetFont(&GUI_Font16B_1);
      GUI_DispStringInRect("µC/GUI Touch screen demo", &rText,
         GUI_TA_HCENTER | GUI_TA_VCENTER);
      ahButton[0] = BUTTON_Create(XMid - 50, YMid - 30, 100, 50, ID_CALIBRATE,
                     BUTTON_CF_SHOW);
      ahButton[1] = BUTTON_Create(XMid - 90, YMid + 30, 80, 30, ID_KEYBOARD,
                     BUTTON_CF_SHOW);
      ahButton[2] = BUTTON_Create(XMid + 10, YMid + 30, 80, 30, ID_TESTCAL,
                     BUTTON_CF_SHOW);
      ...
      case ID_CALIBRATE:
         ...
         //bhs case ID_TESTCAL:   _TestCalibration(); break;
      }
   }
   while (r && (r != 'n') && (r != 'N'));
}
